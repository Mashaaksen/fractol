        ...
        while (x < WIDTH)
        {
            ...
            while (n < ITERATIONS)
            {
                ...
                if (new_z.re + new_z.im > 4)
                    break ;
                stat_z.im = 2 * stat_z.re * stat_z.im + data->fract.julia_im;
                stat_z.re = new_z.re - new_z.im + data->fract.julia_re;
                n++;
            }
            if (new_z.re + new_z.im > 4)
...
int ft_julia(t_fract fract)
{
    ...
    while (i < THREADS)
    {
        ...
        pthread_create(&pthread[i], NULL, draw_julia, (void *)&data[i]);
        i++;
    }
    i = 0;
    while (i < THREADS)
